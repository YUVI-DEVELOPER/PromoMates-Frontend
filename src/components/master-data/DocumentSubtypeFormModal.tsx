...
type DocumentSubtypeFormValues = {
...
type FormErrors = Partial<Record<keyof DocumentSubtypeFormValues, string>>;


type DocumentSubtypeFormModalProps = {
  isOpen: boolean;
  mode: "create" | "edit";
  ...
  onClose: () => void;
  onSubmit: (payload: DocumentSubtypePayload) => Promise<void>;
};
...
function getInitialValues(
  mode: "create" | "edit",
  documentSubtype: DocumentSubtype | null,
): DocumentSubtypeFormValues {
  if (mode === "edit" && documentSubtype) {
    return {
      document_type_id: String(documentSubtype.document_type_id),
      name: documentSubtype.name,
      code: documentSubtype.code,
      description: documentSubtype.description ?? "",
...
export function DocumentSubtypeFormModal({
  isOpen,
  mode,
  documentSubtype,
  documentTypes,
  ...
  onClose,
  onSubmit,
}: DocumentSubtypeFormModalProps) {
  const draftKey = isOpen
    ? mode === "create"
      ? "master-data:document-subtype:create"
      : documentSubtype
        ? `master-data:document-subtype:edit:${documentSubtype.id}`
        : null
    : null;
  const {
    draftState,
    ...
    clearDraft,
    resetDraftState,
  } = useRedisFormDraft<DocumentSubtypeFormValues>(draftKey);
  const [values, setValues] = useState<DocumentSubtypeFormValues>(() =>
    getInitialValues(mode, documentSubtype),
  );
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      const initialValues = getInitialValues(mode, documentSubtype);
  ...
  }, [documentSubtype, isOpen, loadDraft, mode, resetDraftState]);

  function updateValue<FieldName extends keyof DocumentSubtypeFormValues>(
    fieldName: FieldName,
    value: DocumentSubtypeFormValues[FieldName],
  ) {
      ...
      <div className="max-h-full w-full max-w-2xl overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-xl">
        <div className="border-b border-slate-200 px-6 py-4">
          <h2 className="text-xl font-semibold text-slate-950">
            {mode === "create" ? "Create Document Subtype" : "Edit Document Subtype"}
          </h2>
        </div>
        ...
        <form className="space-y-5 px-6 py-5" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <select
              ...
              onChange={(event) => updateValue("document_type_id", event.target.value)}
